#! /usr/bin/env python
# MIE Test that a user with no patient-user relationship can access a restricted patient if he breaks the glass
# Written by Aaron Lehmann
# Date: 02-28-2014 = initial creation

"""
user_access_restriction_no_pur_break_glass.py -- Test that a user with no patient-user relationship can access a restricted patient if he breaks the glass
"""
...
def main (d, WCURL):
    ...
    d.enterFormData( "Updating for user_access_restriction_no_pur_break_glass", id="sec_role_comment" )
    ...
    locator = "//td[font='Allow Emergency Access to charts:']/following-sibling::td[1]/descendant::select/option[@selected]"
    emergency_text = d.getAttribute( 'text', xpath=locator )
    d.reportCommandStatus( "d.getAttribute", locator, emergency_text == "Yes", emergency_text, "selenium user can break the glass" )
#    d.screenshot( )
    if unrestricted_text == "No" and emergency_text == "Yes":
        # Click the 'Control Panel' tab then click the 'Access Control' tab
        d.clickElement( xpath=SIDETAB_XPATH % {'tabtext': 'Control Panel'} )
        d.clickChartTab( top='Access Control' )
        d.enterFormData( "selenium", id="search" )
...
#                d.screenshot( )
                if bool( el ):
                    ...
                    locked_el = d.getElement( xpath=locator )
                    d.reportCommandStatus( "d.getElement", "xpath=%s" % locator, bool( locked_el ), "", "User selenium cannot access William S. Hart because he is locked" )
                    locator = "//a[@onclick='EC_EmergencyAccess_OK();']"
                    break_el = d.getElement( xpath=locator )
                    d.reportCommandStatus( "d.getElement", "xpath=%s" % locator, bool( break_el ), "", "User selenium can break the glass" )
#                    d.screenshot( )
                    if bool( locked_el ) and bool( break_el ):
                        break_el.click( )
                        ...
                        d.clickElement( name="pat_search" )
                        locator = "//a[text()='Hart, William S.']"
                        d.reportCommandStatus( "d.getElement", "xpath=%s" % locator, bool( d.getElement( xpath=locator ) ), "", "User selenium can access William S. Hart's chart." )
#                        d.screenshot( )
