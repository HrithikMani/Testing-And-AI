#! /usr/bin/env python
# MIE Test that a user with partition restriction and no access to an unrestricted patient's partition is denied access
# Written by Aaron Lehmann
# Date: 02-05-2014

"""
user_access_user_has_no_access_to_partition -- Test that a user with partition restriction and no access to an unrestricted patient's partition is denied access
"""
...
def main (d, WCURL):
    ...
    #$ Verify that we can see William S. Hart's chart at the beginning
    # Click 'E-Chart' tab
    ...
    d.reportCommandStatus( "d.getElement", locator, bool( el ), "", "At the start, user selenium is able to access William S. Hart" )
#    d.screenshot( )
    if bool( el ):
        hart_chart_url = d.getAttribute( 'href', xpath=locator ).split( '/' )[-1]
...
#                    d.screenshot( )
                    if not bool( el ):
                        ...
                        el = d.getElement( xpath=locator )
                        d.reportCommandStatus( "d.getAttribute", "xpath=%s" % locator, not bool( el ), "", "Could not find William S. Hart because user selenium does not have access to the MIE partition" )
                        d.navigate( '/' + hart_chart_url )
                        locator = "//div[contains( text(),  'Partition Restrictions' )]"
                        d.reportCommandStatus( "d.getAttribute", "xpath=%s" % locator, bool( d.getElement( xpath=locator ) ), "", "Could not access William S. Hart because user selenium does not have access to the MIE partition" )
#                        d.screenshot()
